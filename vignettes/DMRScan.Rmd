...
```{r}
pos <- matrix(as.integer(unlist(strsplit(names(test.statistics),split="chr|[.]"))), ncol = 3, byrow = TRUE)[,-1] 
head(pos)
```
...
window.thresholds.importantSampling <- estimate_t_grid(k_grid = window.sizes, L = n.CpG, method = "zhang", mcmc = 10000)

## Estimating the window threshold using the closed form expression
window.thresholds.siegmund <- estimate_t_grid(k_grid = window.sizes, L = n.CpG, method = "siegmund")
``` 
...
```{r}
dmrscan.results   <- dmr_scan(obs = regions, k_grid = window.sizes, t_grid = window.thresholds.importantSampling)
## Print the result
