...
## Maxium distance (in base-pairs) within a cluster before it is broken 
## up into two seperate cluster
max.gap <- 750   
>>>>>>> c23fb8d915f4b124ad2c4c58590dfbecad9f03b6
```
...
<<<<<<< HEAD
```R
regions <- makeCpGregions(observations = observations, chr = pos[,1], pos = pos[,2], maxGap = max.gap, minCpG = min.cpg)
=======
```{r}
...
window.thresholds.importantSampling <- estimate_t_grid(k_grid = window.sizes, 
                                          L = n.CpG, method = "zhang", mcmc = 10000)
...
<<<<<<< HEAD
```R
dmrscan.results   <- dmr_scan(obs = regions, k_grid = window.sizes, t_grid = window.thresholds.siegmund)
=======
```{r}
dmrscan.results   <- dmr_scan(obs = regions, k_grid = window.sizes, 
