...
#' @param windowSize The different window sizes to be tested. Must be either one, or an ordered sequence of integers.
#' @param method Gives the method by which the threshold is calculated. Can be either an analytical solution 
#'  "siegmund", provided by Siegnumd et.al (2012), or an iterative process; either importance sampling "sampling", as suggested 
...
#' @param nCPU When calculating the thresholds on a cluster, how many CPUs should be used. This option is only compatible with the 'mcmc' method.
#' @param submethod A character string indicating if an AR(5) or ARIMA model should be used. In the AR(5), the index runs from -2 to 2. 
#' A regular AR(p) model can be obtaine using ARIMA(p,0,0) instead. 
...
#' @examples
#' thresholdGrid <- estimateWindowThreshold(nProbe = 1000, windowSize = 3:8, method = "siegmund")
#' @export
...
estimateThresholdGrid.mcmc <- function(nProbe, windowSize, mcmc, nCPU = 1,submethod = "ar",...){
    ...
    iterGrid   <- as.data.frame(rbind(rep(windowSize,each=length(thresholdGrid)),rep(thresholdGrid,by=length(windowSize))))
    iterGrid   <- do.call(cbind,replicate(mcmc,iterGrid))
    
    if(nCPU == 1){
        cat("For parallel use, set nCPU > 1\n")
        significantWindow <- apply(iterGrid,2,function(x,nProbe,method,...) windowTest(threshold = x[2],windowSize = x[1],nProbe = nProbe,method = submethod,...),nProbe = nProbe,method = submethod,...)
    }else{
...
#        options(MulticoreParam=quote(MulticoreParam(workers=nCPU))) 
       ...
       FUN <- function(x,...)windowTest(threshold  = x$threshold,
        ...
        cl <- makeCluster(nCPU,"PSOCK")
        significantWindow <- unlist(parLapplyLB(cl, iterGrid,FUN,...))
        stopCluster(cl)
    }

    resultArray  <- array(significantWindow,dim=c(length(thresholdGrid),length(windowSize),mcmc))
    ...
    if(any(result == 0)){ 
        ...
        resultSmooth <- apply(logResult,1,function(x,y,df){return(smooth.spline(x=x,y=y,df=df))},y=thresholdGrid,df=2)
...
## arguments mcmc and nCPU is not used, but kept for consistency, and since the
## are automatically pased from estimateThreshold
estimateThresholdGrid.siegmund <- function(windowSize, nProbe, mcmc = 1, nCPU = 1,...){
    ...
    for(i in seq_along(windowSize)){
        ...
        thresholdGridNew <- numeric(length(windowSize))
        resultSmooth     <- apply(log(result),1,smooth.spline,y=thresholdGrid,df=4)
...
estimateThresholdGrid.sampling <- function(windowSize, nProbe, mcmc = 1000, nCPU = nCPU,...){
    ...
    if(useSpline){
        ...
        resultSmooth <- apply(logResult,1,function(x,y,df){u <- is.finite(x);return(smooth.spline(x=x[u],y=y[u],df=df))},y=thresholdGrid,df=3)
        for(i in seq_along(windowSize))
            thresholdGridNew[i]   <- predict(resultSmooth[[i]],x=log(windowSize[i]*lambdaStar))$y
    }
