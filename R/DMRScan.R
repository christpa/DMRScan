...
#' @param windowSize A sequence of windowSizes for the slidingWindow, must be an integer 
#' @param windowThreshold Optional argument with corresponding cut-off for each window. Will be estimated if not supplied.
#' @param ... Optional arguments to be pased to estimate_windowThreshold(), if no grid is specified.
#' @return An object of type RegionList with signficantly differentially methylated regions
#' @keywords DMRScan
#' @export
#' @examples
#' data(DMRScan.methylationData) ## nProbeoad methylation data from chromosome 22
#' data(DMRScan.phenotypes) ## nProbeoad phenotype (end-point for methylation data)
#' 
...
#' regions <- makeCpGregions(observations = test.statistics, chr = positions[,1], pos = positions[,2], maxGap = max.gap, minCpG = min.cpg)
#' 
#' windowSizes <- 3:7 ## Number of CpGs in the slidingWindows, can be either a single number or a sequence of windowSizes
#' nCpG        <- nCpG(regions) ## Number of CpGs to be tested
#' 
#' ## Estimate the windowThreshold, based on the number of CpGs and windowSizes
#' windowThresholds <- estimateWindowThreshold(nProbe = nCpG, windowSize = windowSizes, method = "sampling", mcmc = 10000)
#' ## Run the slidingWindow
#' DMRScanResults   <- dmrscan(observations = regions, windowSize = windowSizes, windowThreshold = windowThresholds)
#' ## Print the result
...
dmrscan <- function(observations,windowSize,windowThreshold=NULL,...){
    ...
    upperBound     <- c(which(zhangWindow)[which(diff(which(zhangWindow))>1)],rev(which(zhangWindow))[1])
   ...
   slidingValues.no.zero  <- na.omit(as.numeric(slidingValues))
   ll                      <- length(slidingValues.no.zero)

   phi                     <- acf(tVal[1,],plot = FALSE)$acf[2:3]
   ...
   mean                    <- median(apply(slidingValues, 1, mean, na.rm = TRUE))
   log.p.val.normal        <- -1*pnorm(abs(tVal[1,]), mean=mean, sd =sd,lower.tail=FALSE,log.p=TRUE)/log(10)
